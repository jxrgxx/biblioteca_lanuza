...
export default function Registro() {
  ...
  useEffect(() => {
    if (modoManual && alumnos.length === 0) {
      api
        .get('/usuarios')
        .then((r) => setAlumnos(r.data.filter((u) => u.rol === 'alumno')));
    }
    if (modoManual) {
      setManualSearch('');
  ...
  const handleScan = async (e) => {
    ...
    try {
      ...
      if (data.rol !== 'alumno') {
        setScanError(
          `El usuario escaneado es ${data.rol}, no alumno. Solo se registran alumnos.`
        );
        setScanInput('');
        return;
      }
      if (!data.ubicacion) {
        setScanError(
          'El alumno no tiene curso asignado. Edita el usuario antes de registrarlo.'
        );
        setScanInput('');
        return;
      }
  ...
  const handleAnadir = async () => {
    ...
    try {
      ...
      await api.post('/registro', {
        nombre,
        codigo_usuario: preview.codigo,
        curso: preview.ubicacion,
        fecha,
      });
      resetScan();
      load();
                ...
                {scanLoading ? (
                  ...
                  <>
                    ...
                    <div className="text-center">
                      <p className="font-bold text-gray-800">
                        {preview.nombre} {preview.apellidos}
                      </p>
                  ...
                  <>
                    ...
                    <p className="text-xs text-gray-400">
                      Escanea de nuevo para reintentar
                    </p>
                  </>
                ) : (
                  <>
                    ...
                    <p className="text-xs text-gray-300">
                      El campo está listo para recibir el código
                    </p>
                  </>
                )}
              </div>
                ...
                <div className="border border-green-200 bg-green-50 rounded-lg px-4 py-3 flex items-center justify-between">
                  <div>
                    <p className="font-semibold text-gray-800 text-sm">
                      {manualPreview.nombre} {manualPreview.apellidos}
                    </p>
                    <p className="text-xs text-gray-500">
                      {manualPreview.ubicacion}
                    </p>
                  </div>
                  <span className="text-xs font-mono text-gray-400">
                    {manualPreview.codigo}
                  </span>
                </div>
              )}
        ...
        <div className="lg:col-span-2 bg-white rounded-xl shadow flex flex-col">
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between px-6 py-3 border-t text-sm text-gray-500">
              <span>
                {sorted.length} entradas — página {page} de {totalPages}
              </span>
              <div className="flex gap-2">
                <button
