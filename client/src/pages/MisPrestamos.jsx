    ...
    <div className="min-h-screen bg-gray-50">
      <header className="bg-brand-700 text-white px-6 py-4 flex items-center justify-between">
        <div>
          ...
          <p className="font-bold">Mis préstamos — {user?.nombre} {user?.apellidos}</p>
        </div>
        <button onClick={handleLogout} className="text-sm text-brand-200 hover:text-white">Cerrar sesión</button>
      </header>
        ...
        <div className="bg-white rounded-xl shadow overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              ...
              {!prestamos.length && (
                <tr><td colSpan={5} className="px-4 py-8 text-center text-gray-400">Sin préstamos</td></tr>
              )}
            </tbody>
          </table>
        </div>
