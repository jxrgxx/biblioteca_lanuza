...
export default function ResetPassword() {
  ...
  if (!token) {
    return (
      <div className="min-h-screen flex bg-brand-600 relative">
        <div className="absolute top-4 right-4 z-10">
          <Link
            to="/"
            className="flex items-center gap-2 bg-white/10 hover:bg-white/20 text-white text-sm font-medium px-4 py-2 rounded-full transition-all backdrop-blur-sm"
          >
            <BookOpen size={15} />
            Ver catálogo
          </Link>
        ...
        />
        <div className="flex-1 flex items-center justify-center bg-brand-600 px-8">
          <div className="bg-white rounded-xl shadow p-8 max-w-md w-full text-center space-y-4">
            <p className="text-gray-700 font-medium">Enlace no válido.</p>
            <Link
              to="/login"
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(
        err.response?.data?.error || 'El enlace no es válido o ha caducado.'
      );
    } finally {
      setLoading(false);
    }
