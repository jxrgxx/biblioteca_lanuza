...
exports.tasaDevolucion = async (req, res) => {
  try {
    ...
    const [[{ total }]] = await db.query(
      `SELECT COUNT(*) AS total FROM prestamo ${where}`,
      params
    );
    const [[{ devueltos }]] = await db.query(
      `SELECT COUNT(*) AS devueltos FROM prestamo ${where} AND devuelto = 1`,
      params
    );
...
exports.alumnosMorosos = async (req, res) => {
  try {
    ...
    if (desde) {
      where += ' AND p.fecha_inicio >= ?';
      dateParams.push(desde);
    }
    if (hasta) {
      where += ' AND p.fecha_inicio <= ?';
      dateParams.push(hasta);
    }
...
exports.resumen = async (req, res) => {
  try {
    ...
    const [[{ visitas }]] = await db.query(
      `SELECT COUNT(*) AS visitas FROM registro ${rw}`,
      rp
    );
